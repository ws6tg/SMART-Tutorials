...
def Mutual_Nearest_Neighbors1(adata, key=None, n_nearest_neighbors=1, farthest_ratio=0.5):
    """
    Simpler version of MNN-based triplet construction (no sampling).
    ...
    n_nearest_neighbors : int, default=1
        Number of nearest neighbors to consider.
    farthest_ratio : float, default=0.5
        Fraction of farthest neighbors to consider for negatives.
    ...
    anchors : list[int]
        Anchor indices.
    positives : list[int]
        Positive indices (MNNs).
    negatives : list[int]
        Negative indices (random farthest neighbors).
    """
    anchors, positives, negatives = [], [], []
    l = adata.shape[0]
    X = adata.X if key is None else adata.obsm[key]
